...
from OCC.Core.gp import gp_Pnt, gp_Dir, gp_Lin, gp_Ax2, gp_Vec
...
# 法線ベクトルの類似性に基づいてクラスタ分け
def cluster_points_normals(points, normals, eps=0.05, min_samples=10):
    db = DBSCAN(eps=eps, min_samples=min_samples, metric="cosine").fit(normals)
    ...
    for label in unique_labels:
        ...
        refined_labels = kmeans.fit_predict(cluster_points)
        for i in range(3):
            clusters.append((cluster_points[refined_labels == i], label))
...
def extract_planes_ransac(
    points, min_samples=50, max_trials=100, residual_threshold=1.0
):
    best_inlier_points = []
    best_num_inliers = 0
    ...
    for _ in range(max_trials):
        ...
        if len(inlier_points) > best_num_inliers:
            best_inlier_points = inlier_points
            best_num_inliers = len(inlier_points)

        # 十分なインライア数が得られた場合、ループを抜ける
        if best_num_inliers >= min_samples:
            break

    outlier_mask = np.logical_not(ransac.inlier_mask_)
    remaining_points = points[outlier_mask]

    return best_inlier_points, remaining_points
...
if __name__ == "__main__":
    ...
    final_shape_with_hole = BRepAlgoAPI_Cut(final_shape_with_hole, hole_sphere).Shape()

    display.DisplayShape(final_shape_with_hole, transparency=0.9)

    # バウンディングボックスの8点から単位球を作成
    unit_sphere_centers = get_bounding_box_corners(final_shape_with_hole, margin=1.5)
    for center in unit_sphere_centers:
    ...
    # 法線ベクトルの類似性に基づいて点群をクラスタ分け
    clusters = cluster_points_normals(points, normals, eps=0.05, min_samples=10)

    # RANSACで平面を抽出（複数試行）
    best_planes = []
    remaining_points = points
    while len(remaining_points) >= 50:  # 50は最低インライア数の例
        best_inlier_points, remaining_points = extract_planes_ransac(remaining_points)
        if len(best_inlier_points) < 50:
            break
        best_planes.append(best_inlier_points)

    # クラスタごとの色を生成
    colors = generate_cluster_colors(len(best_planes))

    # 各平面を表示
    for plane_points, color in zip(best_planes, colors):
    ...
    for point, normal in zip(points, normals):
        start = gp_Pnt(*point)
        vec = gp_Vec(*normal) * scale
        display.DisplayVector(vec, start)
    """
