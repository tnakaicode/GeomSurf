...
from OCC.Core.gp import gp_Pnt, gp_Trsf, gp_Vec, gp_Dir, gp_XYZ, gp_Pln
...
def get_oriented_boundingbox_ratio(shape, optimal_OBB=True, ratio=1.0):
    ...
    anAxes = gp_Ax3(p, gp_Dir(aZDir), gp_Dir(aXDir))
    if dz <= 0.1E-6:
...
if __name__ == '__main__':
    ...
    shp = obj.make_plane_axs(axs, rx=[-100, 100], ry=[-50, 50])
    print(shp.Location().Transformation())
    trf = gp_Trsf()
