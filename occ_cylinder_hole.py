...
if __name__ == '__main__':
    ...
    face = make_face(surf.Cylinder(), 0, np.pi, -50, 50)
    ...
    #
    # while api_face.More():
    #    obj.display.DisplayShape(api_face.Current())
    #    api_face.Next()

    # rim = make_polygon([gp_Pnt(0,0,0), gp_Pnt(20, 0,0), gp_Pnt(30, 0,20), gp_Pnt(0, 0,20)], closed=True)
    # rim_proj = obj.proj_rim_pln(rim, face, gp_Ax3(gp_Pnt(0,0,0), gp_Dir(0,1,0)), 1)
    api_face = BRepBuilderAPI_MakeFace()
    api_face.Init(face)
    api_face.Add(rim)
    api_face.Build()
